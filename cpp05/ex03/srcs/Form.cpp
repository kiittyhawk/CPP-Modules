...
const char  *Form::outputErr::what() const throw()
{
    return ("wasn't created");
}
...
std::ostream    &operator<<(std::ostream &ostream, Form const &data)
{
    if (&data)
    {
        ostream << YELLOW << "Name: " << data.getName() << "\n" \
                << "Form: " << (data.getSigned() ? "signed" : "not signed") << "\n"
                << "Grade to sign: " << data.getGradeSign() << "\n"
                << "Grade to execute: " << data.getGradeExec() << "\n" << RESET;
    }
    else
        ostream << RED << "[Output] Error: Form is empty" << std::endl;
    return (ostream);
}

void    Form::gradeCheck(size_t const &grade, Form const &form) const
{
